...
void runNvMedia_pipeline(WindowBase *window, dwRendererHandle_t renderer, dwSensorHandle_t camera,
                         ...
                         dwContextHandle_t sdk, float32_t framerate, IMAGE_CALLBACK imgCb);

void sig_int_handler(int sig);
void keyPressCallback(int key);

//------------------------------------------------------------------------------
int run_main(IMAGE_CALLBACK imgCb /*int argc, const char **argv*/) {
// int main(int argc, const char **argv)
// {IMAGE_CALLBACK imgCb = NULL; (void)argc; (void)argv;
    //SDK objects
    ...
    runNvMedia_pipeline(window, renderer, cameraSensor, &rawImageProps, &cameraProps, sdk, framerate, imgCb);
                         ...
                         dwContextHandle_t sdk, float32_t framerate, IMAGE_CALLBACK imgCb)
{
    ...
    if (result == DW_SUCCESS) {
        ...
        rgbImageProperties.pxlType = DW_TYPE_UINT8;
        dw_samples::common::GenericSimpleFormatConverter* rgba2rgbConverter = new dw_samples::common::GenericSimpleFormatConverter(rgbaImageProperties, rgbImageProperties, sdk);
        ...
        while (g_run && !window->shouldClose()) {
            ...
            uint64_t frameTimestampUs = rawImageNvMedia->timestamp_us;
            ...
            if (gTakeScreenshot)
            {
...

                {
                    ...
                    cpuData.resize(rgbaImage.prop.width*rgbaImage.prop.height*4);
            ...
            if(imgCb)
            {
                dwImageCUDA* rgbImage = dw_samples::common::GenericImage::toDW<dwImageCUDA>(rgba2rgbConverter->convert(dw_samples::common::GenericImage::fromDW(&rgbaImage)));
                std::vector<uint8_t> cpuData;
                //std::cout << "rgbImage prop " << rgbImage.prop.width << "," << rgbImage.prop.height << std::endl;
                cpuData.resize(rgbImage->prop.width * rgbImage->prop.height * 3);
                cudaMemcpy2D(cpuData.data(), rgbImage->prop.width*3, rgbImage->dptr[0],
                        rgbImage->pitch[0], rgbImage->prop.width*3, rgbImage->prop.height,
                        cudaMemcpyDeviceToHost);
                imgCb(frameTimestampUs, cpuData.data(), rgbImage->prop.width, rgbImage->prop.height);
            }
